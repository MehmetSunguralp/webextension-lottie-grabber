...
// Import browser polyfill for Firefox
let browser: any;
try {
	// Try to import webextension-polyfill for Firefox
	const { browser: polyfillBrowser } = require("webextension-polyfill-ts");
	browser = polyfillBrowser;
} catch (e) {
	// Fallback to global browser object if available
	browser = (globalThis as any).browser;
}

// Detect the environment and create a unified API object
...
// Helper to get the correct action API (action for MV3, browserAction for MV2)
const getActionAPI = () => {
	if (api.action) {
		return api.action;
	...
	if (details.url.includes(".json")) {
		...
		try {
			...
			if (validDotLottie.success) {
				...
				if (manifest && manifest.animations && manifest.animations.length > 0) {
					if (manifest.activeAnimationId) {
						animationValue = animations[manifest.activeAnimationId];
					} else {
						const firstAnimationId = manifest.animations[0]?.id;
						if (firstAnimationId) {
							animationValue = animations[firstAnimationId];
						}
					}
				} else {
					const firstAnimationKey = Object.keys(animations)[0];
					if (firstAnimationKey) {
						animationValue = animations[firstAnimationKey];
					}
				}
...
api.webNavigation.onBeforeNavigate.addListener(onNavigationBeforeNavigateListener);
